...
SELECT
    ...
    sc.priority_level,
    sc.created_at,
    FLOOR(EXTRACT(EPOCH FROM (CURRENT_TIMESTAMP - sc.created_at)) / 60) AS age_minutes,
...
SELECT
    ...
    AVG(EXTRACT(EPOCH FROM (ei.created_at - ec.call_started_at))) AS avg_seconds_call_to_incident
...
SELECT
    ...
    aa.name AS area_name,
    rc.category_code,
    EXTRACT(HOUR FROM ec.call_started_at) AS call_hour,
    COUNT(*) AS call_count
FROM emergency_calls ec
...
LEFT JOIN administrative_areas aa ON aa.id = l.admin_area_id
GROUP BY aa.id, aa.name, rc.category_code, EXTRACT(HOUR FROM ec.call_started_at);

CREATE VIEW vw_escalation_comparison AS
SELECT
    ei.origin_type,
    COUNT(*) AS incident_count,
    AVG(FLOOR(EXTRACT(EPOCH FROM (ei.created_at - ei.reported_at)) / 60)) AS avg_minutes_reported_to_created,
    AVG(FLOOR(EXTRACT(EPOCH FROM (ei.resolved_at - ei.created_at)) / 60)) AS avg_minutes_created_to_resolved
FROM emergency_incidents ei
...
SELECT
    ...
    FLOOR(EXTRACT(EPOCH FROM (ei.created_at - MIN(ec.call_started_at))) / 60) AS call_to_incident_minutes,
    FLOOR(EXTRACT(EPOCH FROM (MIN(iap.joined_at - ei.created_at)) / 60)) AS incident_to_agency_minutes,
    FLOOR(EXTRACT(EPOCH FROM (MIN(d.dispatched_at - MIN(iap.joined_at))) / 60)) AS agency_to_dispatch_minutes,
    FLOOR(EXTRACT(EPOCH FROM (MIN(d.arrived_at - MIN(d.dispatched_at))) / 60)) AS dispatch_to_arrival_minutes
FROM emergency_incidents ei
        ...
        SUM(
            CASE
                WHEN lv.verdict IN ('false_alarm', 'malicious_false_report')
                    AND lv.created_at >= (CURRENT_TIMESTAMP - INTERVAL '30 days')
                THEN 1
                ELSE 0
            END
