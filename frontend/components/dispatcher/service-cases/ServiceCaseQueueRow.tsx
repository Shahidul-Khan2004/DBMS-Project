...
interface ServiceCaseQueueRowProps {
  serviceCase: OperationsServiceCase;
}

export function ServiceCaseQueueRow({ serviceCase }: ServiceCaseQueueRowProps) {
  ...
  return (
    <li>
      <Link
        href={href}
        className={`group block rounded-xl border px-4 py-3 focus-visible:outline focus-visible:outline-2 focus-visible:outline-offset-[-2px] focus-visible:outline-[#002D62] ${getServiceCaseCardAccent(serviceCase.priority_level)} ${getDispatcherClickableCardRowClasses()}`}
      >
