...
import { ReliefLineItemsEditor, parseReliefLineItems, type ReliefLineItemRow } from "@/components/admin/disasters/detail/ReliefLineItemsEditor";
...
import { postAgencyDisasterReliefRequest } from "@/lib/agency-disaster-api";
...
type AgencyCreateReliefRequestModalProps = {
  open: boolean;
...
export function AgencyCreateReliefRequestModal({
  open,
  disasterPublicUuid,
  activeShelters,
  onClose,
  onSuccess,
}: AgencyCreateReliefRequestModalProps) {
  ...
  const handleSubmit = async (event: FormEvent) => {
    ...
    setSubmitError(null);
    try {
      await postAgencyDisasterReliefRequest(disasterPublicUuid, {
    ...
    <ModalPortal open={open}>
      ...
      >
        ...
        <div className="min-h-0 flex-1 space-y-4 overflow-y-auto px-5 py-4">
          <div>
            <FieldLabel htmlFor="agency-request-shelter" required>
              Shelter
            </FieldLabel>
            <select
              id="agency-request-shelter"
              value={shelterUuid}
            ...
            >
              {activeShelters.length === 0 ? (
                <option value="">
                  No active agency-managed shelters for this disaster.
                </option>
              ) : (
          ...
          />
          <div>
            <FieldLabel htmlFor="agency-request-note">Request note</FieldLabel>
            <textarea
              id="agency-request-note"
              value={requestNote}
