...
import { updateAgencyDispatchStatus } from "@/lib/agency-api";
import {
  getDispatchConfirmTitle,
...
const SUCCESS_TOASTS: Record<AgencyDispatchStatusAction, string> = {
...
type AgencyDispatchStatusModalProps = {
  open: boolean;
  dispatch: AgencyDispatch | null;
  targetStatus: AgencyDispatchStatusAction | null;
  onClose: () => void;
  onSuccess: () => Promise<void>;
};

export function AgencyDispatchStatusModal({
  open,
  dispatch,
  targetStatus,
  onClose,
  onSuccess,
}: AgencyDispatchStatusModalProps) {
  ...
  const canSubmit =
    Boolean(dispatch?.public_uuid && targetStatus) &&
  ...
  useEffect(() => {
    ...
    setNote("");
    setSubmitError(null);
  }, [open, dispatch?.public_uuid, targetStatus]);
  ...
  const handleSubmit = async () => {
    if (!dispatch?.public_uuid || !targetStatus || !canSubmit) return;

    setIsSubmitting(true);
    setSubmitError(null);

    try {
      await updateAgencyDispatchStatus(dispatch.public_uuid, {
      ...
      if (process.env.NODE_ENV === "development") {
        console.error("Failed to update agency dispatch status", err);
      }
  ...
  const title = getDispatchConfirmTitle(targetStatus);
  const actionLabel = getDispatchActionLabel(targetStatus);
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/40 px-4">
      <div
        ...
        role="dialog"
        aria-labelledby="agency-dispatch-status-title"
        aria-modal="true"
      >
        <div className="border-b border-slate-100 px-5 py-4">
          <h2
            id="agency-dispatch-status-title"
            className="text-lg font-semibold text-slate-900"
          >
            {title}
          </h2>
          <p className="mt-2 text-sm text-slate-600">
            <span className="font-medium text-slate-800">{dispatch.unit.unit_name}</span>
            <span className="text-slate-400"> · </span>
            {dispatch.incident.incident_code}
          </p>
          {isCancellation ? (
            <p className="mt-2 text-sm leading-6 text-amber-800">
              This will end this unit assignment and return the unit to available status.
            </p>
          ) : null}
        </div>
